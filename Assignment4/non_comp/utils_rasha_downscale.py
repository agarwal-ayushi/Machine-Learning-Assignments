...
class CaptionsPreprocessing():
    ...
    def captions_transform(self, img_caption_list):
        ...
        # Uncomment this to choose a smaller dataset - random caption from 5 golden captions
        # j = np.random.choice(len(img_caption_token))  # Randomly choose one description out of 5
      #  img_caption_token = img_caption_token[j]
...
class ImageCaptionsDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.img_transform:
            t_ = transforms.Resize(224)
            t = transforms.RandomCrop(112, padding =4)
            image = t(image)
...
class Device_Loader():
        ...
        def __len__(self):
            return len(self.dl)
'''
Collate function for random choosing of captions. 
def collate_fn(data):
...
def collate_fn(data):
    ...
    lengths = [len(cap) for cap in captions]
    # Change the length of all the captions in one batch to the max of all the captions. (Should be across the dataset)? 
    targets = torch.zeros(len(captions), max(lengths)).long()
    for i, cap in enumerate(captions):
        end = lengths[i]
        targets[i, :end] = cap[:end]        
...
def get_data(batch):
   ...
   # Num_workers = 0 for running on Colab else gives mem issue
...
def get_private_test_data():
    ...
    private_test_loader = DataLoader(private_test_dataset, batch_size=1, shuffle= True, num_workers=0)
